...
public class FirstLevel : MonoBehaviour {
	...
	void Start () {
		...
		playerStart = new Vector3(Random.Range(-8, 9), Random.Range(-5, 6) , 0);
		while (true) {
		...
		SecondLevel ();
		Box (24);
		range = new Vector3(Random.Range(-8, 9), Random.Range(-5, 6) , 0);
		while (true) {
	...
	private void Box (int howMany) {
		...
		while (times < howMany) {
			...
			temprange = new Vector3 (Random.Range (-8, 9), Random.Range (-5, 6), 0);
			while (true) {
	...
	/// <summary>
	/// Places the object that takes the player to the second level on a random place with Random.Range witch range is specified on the code.
	/// Makes sure that it is not placed on top of another object.
	/// </summary>
	private void SecondLevel () {
		Vector3 temprange = new Vector3(Random.Range(-8, 9), Random.Range(-5, 6) , 0);
		while (true) {
			if (coordinates.Contains (temprange)) {
				temprange = new Vector3 (Random.Range(-8, 9), Random.Range(-5, 6) , 0);
			} else {
				secondLevel.transform.Translate (temprange);
				coordinates.Add (temprange);
				break;
			}
		}
