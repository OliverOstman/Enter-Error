...
public class FourthLevel : MonoBehaviour {
	...
	void Start () {
		...
		lastLevel = GameObject.Find ("Last Level");
		playerStart = new Vector3(Random.Range(-11, 12), Random.Range(-4, 5) , 0);
		while (true) {
		...
		LastLevel ();
		Box (34);
		range = new Vector3(Random.Range(-11, 12), Random.Range(-4, 5) , 0);
		while (true) {
	...
	private void Box (int howMany) {
		...
		while (times < howMany) {
			...
			temprange = new Vector3 (Random.Range (-11, 12), Random.Range (-4, 5), 0);
			while (true) {
	...
	/// <summary>
	/// Places the object that takes the player to the last level on a random place with Random.Range witch range is specified on the code.
	/// Makes sure that it is not placed on top of another object.
	/// </summary>
	private void LastLevel () {
		Vector3 temprange = new Vector3(Random.Range(-11, 12), Random.Range(-4, 5) , 0);
		while (true) {
			if (coordinates.Contains (temprange)) {
				temprange = new Vector3 (Random.Range(-11, 12), Random.Range(-4, 5) , 0);
			} else {
				lastLevel.transform.Translate (temprange);
				coordinates.Add (temprange);
				break;
			}
		}
